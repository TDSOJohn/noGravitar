...
SolarSystem::SolarSystem(int* scr, const ResourceHolder& resources) : ourHero(resources, Textures::Spaceship, sf::Vector2f(Settings::MAP_X/2, Settings::MAP_Y/3)), ssBackgroundSprite(*(resources.get(Textures::ssBackground).texture)), solarSystemStatus(-1), movingUp(false), movingDown(false), movingLeft(false), movingRight(false), isGrabbing(false), isShooting(false), score(scr)
{
    int tempY =0;
    int i =0;
